...
Nibbler is one of the best open-source GUI's for chess game and position analyis. The program's built-in API for the Lichess games data makes an opening book an unnecessary luxury if you are below the master level. The screenshot shows the engine line and current depth of analysis. At the end of the line, the Lichess API shows the win percenntage, the frequency the move is played, and the number of games in the live, Lichess Masters database. 
